...
from util import (read_json, load_sparse, compute_background_log_frequency)
...
class VAMPIRE(Model):
    ...
    def __init__(self,
        ...
        if reference_vocabulary is not None:
            ...
            self._ref_interaction = np.dot((self._ref_doc_counts).T, self._ref_doc_counts)
            self._ref_doc_sum = np.array(self._ref_doc_counts.sum(0).tolist()[0])
            logger.info("Generating npmi matrices.")
            (self._npmi_numerator, 
             self._npmi_denominator) = self.generate_npmi_vals(interactions=self._ref_interaction, 
                                                                document_sums=self._ref_doc_sum)
    ...
    def forward_on_instances(self,
        ...
        with torch.no_grad():
            ...
            for i, instance in enumerate(dataset.instances):
                # Get the output from the forward method
                # print("type:", type(instance.__getitem__('tokens')))
                # print("token:", instance.__getitem__('tokens').__getitem__(10))
                # padding_lengths = instance.__getitem__('tokens').get_padding_lengths()
                # tokens_tensor = instance.__getitem__('tokens').as_tensor(padding_lengths)['tokens']
                # tokens_tensor = tokens_tensor.unsqueeze(1)
                # print("tensor:",tokens_tensor.shape)

                # test text_fields change into token_ids
                tokens = instance.__getitem__('tokens')
                token_indexer = SingleIdTokenIndexer(namespace=self.vocab_namespace)
                vocab = self.vocab
                token_indices = token_indexer.tokens_to_indices(tokens, vocab, index_name='tokens')
                token_ids = torch.IntTensor(token_indices["tokens"]).unsqueeze(0)
                # padding_lengths = instance.__getitem__('tokens').get_padding_lengths()
                # tensors = instance.__getitem__('tokens').as_tensor(padding_lengths)
                Tokens = token_ids
                tokens_dict = {'tokens':Tokens}
                if isinstance(tokens_dict, dict):
                    embedded_tokens = (self._bag_of_words_embedder(tokens_dict['tokens'])
                               .to(device=self.device))
                variational_output = self.vae(embedded_tokens)

                # Save the output in the current dictionary with a key generated from the index
                instance_separated_output[i] = variational_output
...
@Predictor.register("Vampire_Predictor")
class vampire_predictor(Predictor):

    def __init__(self, model: VAMPIRE, dataset_reader: VampireReader) -> None:
        self._model = model
    ...
    def _json_to_instance(self, json_dict: JsonDict) -> Instance:
        ...
        # id_ = json_dict['id']
        # tokens: a list contains every word in text
        tokens = tokenizer.tokenize(text)
        token_indexer = SingleIdTokenIndexer(namespace="vampire")
        # vocab = self._model.vocab
        # token_indices = token_indexer.tokens_to_indices(tokens, vocab, index_name='tokens')
        # Create fields for the Instance
        # token_ids = np.array(token_indices["tokens"])
        ...
        # array_field = ArrayField(token_ids)
        # print("padding_lengths:", array_field.get_padding_lengths())
        # print("length of text_field", len(text_field))
        # print("length of metadata_field", len(metadata_field))
        # print("metadata_field:", metadata_field.items())
        # Create and return the Instance
        # fields = {'tokens':array_field}
